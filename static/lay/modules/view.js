        ...
        return str.replace(new RegExp('(^' + symbol + '*)', 'g'), '')
            ...
            self.loadHtml = function (url, callback) {
                ...
                $.ajax({
                    ...
                    error: function (res) {
                        self.log(
                            'Solicitar exceção de visualização de arquivo\n caminho do arquivo:' + url + '\n estado: ' + res.status
                        )
                        loadBar.error()
                    }
                })
                ...
                init: function () {
                    ...
                    layui.dropdown.render({
                        ...
                        options: [
                            {
                                name: 'refresh',
                                title: 'Atualize a página atual'
                            },
                            {
                                name: 'other',
                                title: 'Fechar outras guias'
                            },
                            {
                                name: 'all',
                ...
                var defaultParams = {
                    ...
                    error: function (res) {
                        if (res.status == conf.logoutHttpCode) {
                            ...
                            layer.msg('Por favor, verifique sua conexão à internet')
                            self.log('Verifique sua conexão de rede, mensagem de erro:' + JSON.stringify(res))
                        }
