/**
 ...
 @Author：beijiyi
 @version: 0.1
 url: https://gitee.com/beijiyi/tree_table_treegrid_based_on_layui
...
layui.config({}).extend({}).define(['laytpl', 'laypage', 'layer', 'form'], function (exports) {
    ...
    Class.prototype.resetDataresetRoot = true; //是否重新Sim根节点
    /**
     * Sim根节点id(重新登录根节点)
     */
    ...
    Class.prototype.resetDatas = function (list) {
        ...
        if (options.isTree) { //树状
            ...
            table.setDataList(that.config.id, tableList);
            var rootList = table.cache[options.id].data.upIds || []; //根节点list集合
            if (rootList.length <= 0 || that.resetDataresetRoot) { //Sim根节点
                table.cache[options.id].data.upIds = [];
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (!res[response.dataName]) { //返回是未定义或null时转成[]
                        ...
                        res[response.msgName] = 'O status dos dados retornados é anormal';
                    }
                    ;
                    ...
                    if (res[response.statusName] != response.statusCode) {
                        that.renderForm();
                        that.layMain.html('<div class="' + NONE + '">' + (res[response.msgName] || 'O status dos dados retornados é anormal') + '</div>');
                    } else {
                ...
                error: function (e, m) {
                    that.layMain.html('<div class="' + NONE + '">Falha na solicitação de dados</div>');
                    that.renderForm();
                    loadIndex && layer.close(loadIndex);
    ...
    Class.prototype.loading = function () {
        ...
        if (options.loading && options.url) {
            return layer.msg('Carregando dados...', {
                icon: 16,
                offset: [
    ...
    //Eventos que são executados apenas uma vez
