 ...
 * @see baidu.dom.q
 *
 * @return {HTMLElement|null} 获取的元素，查找不到时Retornarnull,如果参数不合法，直接Retornar参数.
 */
 ...
 * @param {Object} [thisObject] 函数调用时的this指针，如果没有此参数，默认Sim当前遍历的数组
 * @remark
...
/**
 * 判断目标参数SimNão为function或Function实例
...
/**
 * opera 从10开始不Sim用opera后面的字符串进行版本的判断
 * 在Browser identification最后Novo RegistroVersion + 数字进行版本标识
 * opera后面的数字保持在9.80不变
 */
 ...
 * @param {boolean} options.menu 				SimNão显示右键菜单，允许值：true/false
 * @param {boolean} options.loop 				播放到最后一帧时SimNão重新播放，允许值： true/false
 * @param {boolean} options.play 				flashSimNão在浏览器加载时就开始播放。允许值：true/false
 ...
 * @param {boolean} options.allowfullscreen 	SimNão允许flash全屏。允许值：true/false
 * @param {boolean} options.seamlesstabbing 	允许设置执行无缝跳格，从而使用户能跳出flash应用程序。该参数只能在安装Flash7及更高版本的Windows中使用。允许值：true/false
 * @param {boolean} options.devicefont 			设置静态文本对象SimNão以设备字体呈现。允许值：true/false
 * @param {boolean} options.swliveconnect 		第一次加载flash时浏览器SimNão应启动Java。允许值：true/false
 ...
 * @param {string} name flash对象的Nome
 * @see baidu.swf.create
 * @meta standard
...
baidu.flash._Base = (function(){
    ...
    return function(options, callBack){
         ...
         * @param {Function} [callBack] 异步调用后将Retornar值作为参数的调用回调函数，如无Retornar值，可以不传入此参数
         * @return {Null}
        */
 ...
 * @config {Number} vars.mode mode == 0时，Sim使用滚动条，mode == 1时，拉伸flash, 默认值为0
 ...
 * @param {String|Function} deleteFileCallback Exluir文件的回调
...
/**
 * Exluirscript的属性，再删除script标签，以解决修复内存泄漏的问题
 * 
 * @param {HTMLElement} src script节点
 */
 ...
 * 2、如果请求了一个不存在的页面，callback函数在IE/opera下也会被调用，因此使用者需要在onsuccess函数中判断数据SimNão正确加载。
 * @meta standard
...
baidu.sio.callByServer = /**@function*/function(url, callback, opt_options) {
    ...
    /*
     * Retornar一个函数，用于立即（挂在window上）或者超时（挂在setTimeout中）时执行
     */
...
baidu.json.parse = function (data) {
    //2010/12/09：更新至不使用原生parse，不检测用户输入SimNão正确
