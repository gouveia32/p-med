...
func (c *BaseController) SaveFile(PIC_PATH string, AllowExt map[string]bool) (fpath string, error error) {
	...
	if _, err := AllowExt[ext]; !err {
		return "", errors.New("O formato do arquivo de upload está incorreto")
	}
	//创建保存目录

	uploadDir := PIC_PATH + time.Now().Format("2021/12/29/")
	...
	hashName := SycMd5(time.Now().Format("2021_12_29_15_04_05_") + randNum)
	fileName := fmt.Sprintf("%x", hashName) + ext
	fpath = uploadDir + fileName
	defer f.Close() //Feche o arquivo carregado, caso contrário, o arquivo temporário não pode ser limpo
