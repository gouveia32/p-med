...
var cpt *captcha.Captcha //Código de verificação
func init() {
...
func (c *LoginController) Login() {
	if c.IsPost() {
		...
		valid.Required(c.GetString("username"), "login").Message("Usuário")
		valid.Required(c.GetString("password"), "password").Message("Senha")
		if valid.HasErrors() {
		...
		if err != nil {
			...
			userInfo["user"] = user
			c.Abort200(userInfo, "login bem sucedido", "/")
		}
	} else {
